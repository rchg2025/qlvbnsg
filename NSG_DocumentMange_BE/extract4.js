...
// Now parse the arrays from userContent string
const regex = /\[\s*\{\s*"_id"/g;
let match;
const arrays = [];
...
while ((match = regex.exec(userContent)) !== null) {
    ...
    for (let i = startIndex; i < userContent.length; i++) {
    ...
    if (endIndex !== -1) {
        arrays.push(userContent.substring(startIndex, endIndex + 1));
